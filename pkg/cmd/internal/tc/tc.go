...
func (c *Controller) Init() error {
	for _, ifce := range c.interfaces {
		_, _ = exec.Command("/bin/sh", "-c", fmt.Sprintf("sudo tc qdisc del dev %s root", ifce)).Output()
		out, err := exec.Command("/bin/sh", "-c", fmt.Sprintf("sudo tc qdisc add dev %s root handle %d: htb default %d",
		...
		out, err = exec.Command("/bin/sh", "-c", fmt.Sprintf("sudo tc class add dev %s parent %d: classid %d:%d htb rate 100mbit",
...
func (c *Controller) AddLatency(srcIP, dstIP string, latency time.Duration) error {
	...
	for _, ifce := range c.interfaces {
		...
		out, err = exec.Command("/bin/sh", "-c", fmt.Sprintf("sudo tc qdisc add dev %s parent %d:%d handle %d: netem delay %v",
			ifce, rootHandle, class, handle, latency)).Output()
		if err != nil {
			return errors.Wrapf(err, "failed to add tc netem delay of %v: %s", latency, out)
		}
		out, err = exec.Command("/bin/sh", "-c", fmt.Sprintf("sudo tc filter add dev %s parent %d: protocol ip u32 match ip src %s/32 match ip dst %s/32 flowid %d:%d",
...
func (c *Controller) CleanUp() error {
	var errs []string
	for _, ifce := range c.interfaces {
		out, err := exec.Command("/bin/sh", "-c", fmt.Sprintf("sudo tc qdisc del dev %s root", ifce)).Output()
		if err != nil {
