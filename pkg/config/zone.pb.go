...
func NewPopulatedZoneConfig(r randyZone, easy bool) *ZoneConfig {
	...
	v3 := NewPopulatedGCPolicy(r, easy)
	this.GC = *v3
	this.NumReplicas = int32(r.Int31())
	if r.Intn(2) == 0 {
		this.NumReplicas *= -1
	...
	if r.Intn(10) != 0 {
		v4 := r.Intn(5)
		this.Constraints = make([]Constraints, v4)
		for i := 0; i < v4; i++ {
			v5 := NewPopulatedConstraints(r, easy)
			this.Constraints[i] = *v5
		}
	}
	if r.Intn(10) != 0 {
		v6 := r.Intn(5)
		this.SubzoneSpans = make([]SubzoneSpan, v6)
		for i := 0; i < v6; i++ {
			v7 := NewPopulatedSubzoneSpan(r, easy)
			this.SubzoneSpans[i] = *v7
		}
	}
	if r.Intn(10) == 0 {
		v8 := r.Intn(5)
		this.Subzones = make([]Subzone, v8)
		for i := 0; i < v8; i++ {
			v9 := NewPopulatedSubzone(r, easy)
			this.Subzones[i] = *v9
		}
	}
	if !easy && r.Intn(10) != 0 {
...
func NewPopulatedSubzone(r randyZone, easy bool) *Subzone {
	...
	v10 := NewPopulatedZoneConfig(r, easy)
	this.Config = *v10
	if !easy && r.Intn(10) != 0 {
	}
	return this
...
func NewPopulatedSubzoneSpan(r randyZone, easy bool) *SubzoneSpan {
	...
	if r.Intn(10) != 0 {
		v11 := r.Intn(100)
		this.Key = make(github_com_cockroachdb_cockroach_pkg_roachpb.Key, v11)
		for i := 0; i < v11; i++ {
	...
	if r.Intn(10) != 0 {
		v12 := r.Intn(100)
		this.EndKey = make(github_com_cockroachdb_cockroach_pkg_roachpb.Key, v12)
		for i := 0; i < v12; i++ {
...
func randFieldZone(dAtA []byte, r randyZone, fieldNumber int, wire int) []byte {
	...
	case 0:
		...
		dAtA = encodeVarintPopulateZone(dAtA, uint64(v14))
	case 1:
		dAtA = encodeVarintPopulateZone(dAtA, uint64(key))
...
var fileDescriptorZone = []byte{
	...
	,
}
